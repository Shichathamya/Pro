"""
07_Test_Performance.py
====================
Test Performance of Random_Forest_AC_Plant1
Plant 1 and Plant 2
Performance Ratio per Inverter
"""
...
model        = joblib.load(os.path.join(MODEL_DIR, "Random_Forest_AC_Plant1.pkl"))
feature_cols = joblib.load(os.path.join(MODEL_DIR, "AC_Plant1_features.pkl"))

print(f"  Model    : Random_Forest_AC_Plant1.pkl")
...
def inverter_summary(df):
    ...
    inv["STATUS"] = inv["PERF_RATIO"].apply(
        lambda x: "!! MAINTENANCE" if x < THRESHOLD else "OK"
    )
    return inv
...
def plot_performance(df, inv, plant_num):
    ...
    fig.suptitle(f"Plant {plant_num} — Performance Test  "
                 f"(Model: Random_Forest_AC_Plant1)",
    ...
    # 2. Actual vs Predicted per Inverter
    ...
    fig.savefig(os.path.join(OUTPUT_DIR, f"TestPerf_P{plant_num}.png"),
...
def export_csv(inv, plant_num):
    path = os.path.join(OUTPUT_DIR, f"TestPerf_P{plant_num}_inverter.csv")
...
print("\n" + "#"*60)
print("#  TEST PERFORMANCE -- Random_Forest_AC_Plant1")
print("#"*60)
